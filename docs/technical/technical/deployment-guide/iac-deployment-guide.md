...
1. Once ArgoCD has been installed (in the terminal, you should see something like `TASK [mojaloop.iac.cc_k8s : Install argocd]`), wait a few minutes, and then you can start monitoring events in ArgoCD via port forwarding.
   ...
   1. Obtain the ssh key.
      ...
      1. In a new terminal, navigate to where the ssh key has just been saved. Change the permission of the file so that only you can read it:

         ```bash
         chmod 400 <PATH_TO_SSH_KEY>
         ```

   1. In another terminal, connect to the bastion via ssh:

         ```bash
         ssh -i <PATH_TO_SSH_KEY> ubuntu@<BASTION_IP_ADDRESS>
         ```
   ...
   1. In a new terminal, create a secure tunnel so that visiting `localhost:8080` on your computer connects to `localhost:8080` on the remote server.
      ...
      ```bash
      ssh -i <PATH_TO_SSH_KEY> -L 8080:localhost:8080 ubuntu@<BASTION_IP_ADDRESS>
      ```
...
1. Create a `values-hub-provisioning-override.yaml` file and configure it as follows: <!-- QUESTION: Do we need to create the same file under default-config? -->
...
<!-- QUESTION: When do you have to run refresh-templates? When you start using profiles for the first time? When you start referencing a whole different profile in the submodule.yaml? When you just update something in the already existing profile, do you have to run it? -->
...
1. Once logged in to the Business Portal, update the access rights of the Portal Admin user so that you can use this same user account to access all portals: <!-- QUESTION: Is this needed? Or should this step be removed? Maybe it's not recommended and we only used it in the call to save time? -->
...
<!-- EDITORIAL COMMENT: Why is this needed? For simulating DFSPs, e.g. for e2e testing? If this is the case, would adding a new DFSP in MCM suffice, or would there be other required steps? -->
