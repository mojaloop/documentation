...
The **Account Lookup Service** (**ALS**) _(refer to section `6.2.1.2`)_ as per the [Mojaloop {{ $page.frontmatter.version }} Specification](/api) implements the following use-cases:
...
#### Notes
- `partyIdType` - Values are currently seeded as per section _`7.5.6`_ [Mojaloop {{ $page.frontmatter.version }} Specification](./api).
