...
3. Le DFSP payeur exécute la demande R2P par un appel **POST** `/requestToPayTransfer`. Si le type d’authentification n’est pas fourni dans cet appel, le flux suppose que le payeur confirmera le transfert et les conditions via un **PUT** `/requestToPayTransfer` ; sinon, le flux d’authentification approprié est exécuté.
