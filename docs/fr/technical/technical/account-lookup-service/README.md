...
- `currency` — voir la section `7.5.5` de la spécification Mojaloop ; code selon [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html). Optionnel ; une configuration « par défaut » est requise si aucune devise n’est fournie ou si seule la configuration d'endpoint « par défaut » existe.
- `endPointType` — identifiant du type d'endpoint (ex. `URL`) pour permettre d’autres transports plus tard.
