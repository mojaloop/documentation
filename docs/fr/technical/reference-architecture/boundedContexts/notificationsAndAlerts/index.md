...
- Chaque commande de Notification contient :
  ...
  - une configuration de nouvelle tentative et de livraison précisant comment le BC Notifications doit gérer les échecs de livraison
...
| Terme | Description |
|---|---|
| **Notification** | Notification sortante envoyée par le BC Notifications et Alertes, généralement vers une destination externe, contenant des en-têtes contextuels et des payloads. Un exemple en est les callbacks FSPIOP dans le cadre de la spécification d’API Mojaloop. |
...
- Informations d'endpoint / de transport pour la livraison de la notification
- Modèle pour l’alerte de notification qui sera utilisé pour générer la notification réelle
