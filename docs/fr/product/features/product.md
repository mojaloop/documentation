...
6.  Désactiver un DFSP participant (bien qu’il ne devrait pas être possible de désactiver un DFSP avec des transactions en cours / non réglées).
...
-   Identifiant de lot de règlement
...
Cela inclura l’identifiant de fenêtre / lot de règlement, qui est lui-même cliquable pour permettre à l’opérateur de consulter le statut de règlement du lot et donc de la transaction elle-même.

## Règlement

La gestion du règlement sur le Hub Mojaloop doit être robuste et fiable. Les fonctionnalités associées :
...
3.  Générer automatiquement tous les fichiers de règlement nécessaires pour l’intégration avec le ou les partenaires de règlement lorsqu’une fenêtre de règlement est clôturée.
4.  Consulter les positions de tous les participants dans la fenêtre / le lot.
5.  Une fois le règlement achevé / finalisé, mettre à jour automatiquement les positions et la liquidité disponible actuelle sur la base des rapports du ou des partenaires de règlement.
6.  Fournir des outils pour prendre en charge l’intégration entre le Hub Mojaloop et le ou les partenaires de règlement.

Notez qu’au moins une fenêtre ou un lot de règlement sera toujours ouvert, et les transactions y seront ajoutées au fur et à mesure qu’elles sont traitées. La création d’une nouvelle fenêtre est donc automatique à la clôture de la précédente.
...
1.  Un Hub Mojaloop prend en charge par défaut toutes les devises définies par l’ISO. Chacune est activée pour une utilisation par un déploiement particulier par la création de comptes de règlement et de position pour cette devise. Pour appuyer cela, il est nécessaire de pouvoir consulter les soldes des comptes d’exploitation du Hub (règlement et position, répliqués par devise prise en charge).
