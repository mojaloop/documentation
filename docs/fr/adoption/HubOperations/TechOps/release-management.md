...
La gestion des versions gère les processus de gestion, de planification, de programmation et de contrôle d'une modification logicielle à travers le déploiement et les tests dans divers environnements.
...
Les pratiques standard de développement et de QA – suivies par l'équipe de développement/livraison produit Mojaloop – incluent les éléments suivants dans le cadre de la Définition de fini. La recommandation est que l'opérateur du Hub adopte une stratégie similaire.
...
* Les notes de version ont été créées avec les détails suivants :
    ...
    * Mise en évidence de tout breaking change impactant toute fonctionnalité, solution API ou architecture système
...
* Toutes les mises à jour de maintenance des produits, les nouvelles fonctionnalités et les corrections de bogues sont mises à disposition des clients DFSP des extensions/produits supplémentaires dans le cadre du cycle de publication, une fois par période PI.

## Bogues et hotfixes

Les bogues et hotfixes sont traités de la manière suivante :

* Toutes les corrections de bogues (à la fois Mojaloop et autres produits) sont incluses dans les packages de version.
* De même, les hotfixes sont également fournis via une version. Il n'est pas recommandé de déployer des hotfixes directement à partir des versions de packages d'applications spécifiques, car le déploiement d'un seul composant d'une version (par opposition au déploiement de la version qui inclut le composant mis à jour) peut entraîner une désynchronisation du Hub avec la version du package d'application.
