...
Les invariants suivants ont été définis au fil du développement de la plateforme à partir des exigences techniques déduites des [principes du Level One Project](https://www.leveloneproject.org/project_guide/level-one-project-design-principles/) et des bonnes pratiques du secteur applicables.
...
#### Notes :
1. La plateforme permet aux participants de créditer immédiatement leurs clients tout en minimisant les risques et coûts associés.
2. La plateforme prend en charge des contrôles de liquidité disponible par transfert lorsque nécessaire au premier objectif.
3. Le hub est optimisé pour le chemin critique.
4. Règlement automatisé intrajournalier ; configuré par le schéma et mis en œuvre selon les modèles de règlement recommandés pour l’infrastructure des marchés financiers.
...
2. Le traitement de bout en bout réduit les erreurs humaines dans le processus de transfert et donc les coûts.
...
#### Notes :
1. Lorsqu’un transfert est finalisé, le statut de ce transfert ne peut faire l’objet d’aucun doute (sinon il n’est pas finalisé et un avis actif est communiqué aux participants. L’avis est à la demande : en cas de demande, ils sont informés que le statut est indéterminé).
...
3. Le déterminisme signifie que chaque transfert est traçable, auditable (selon limites et contraintes), avec un résultat final fourni dans un délai garanti.
...
2. Isole la complexité des cas d’usage hors du chemin critique.
...
2. Des services API auxiliaires pour l’intégration, la gestion de position, les rapports de rapprochement et d’autres fonctions non temps réel hors traitement des transferts.
...
5. Pour disposer d’un mécanisme unique pour les requêtes (transactions de masse, saisie utilisateur, sauts multiples, etc.).
6. Pour mieux refléter les réseaux réels : les problèmes de vitesse ou de fiabilité pour un participant doivent avoir un impact minimal sur les autres ou sur la disponibilité globale.
...
1. Les requêtes dupliquées peuvent être émises en toute sécurité par l’émetteur en cas de réseau dégradé.
    1. Les doublons sont reconnus et produisent le même résultat (doublons valides) ou sont rejetés comme doublons (lorsque la spécification ne l’autorise pas) avec référence à l’original.
...
#### Notes :
1. La disponibilité des enregistrements peut être en retard sur la finalité en ligne pour séparer la conservation des données du traitement temps réel des requêtes de transfert.
...
1. La confidentialité protège la vie privée des participants et de leurs clients.
    1. De nombreux domaines réglementaires où Mojaloop doit opérer imposent des obligations légales ; le hub doit appliquer les bonnes pratiques pour cette confidentialité.
...
1. Connexions sécurisées : mTLS obligatoire pour toutes les communications entre le schéma et les participants autorisés.
    1. Garantit confidentialité, correspondants connus et protection contre la falsification.
...
2. Voir la représentation [Level One Decimal Type](https://docs.mojaloop.io/documentation/discussions/decimal.html) et ses formes.
    1. Cette spécification permet l’échange sans perte de précision avec les systèmes financiers XML.
...
5. Le coût du scale-out par unité de capacité doit être inférieur au coût du provisionnement initial.
...
1. Définition de « haute disponibilité » :
   ...
   2. Les schémas peuvent définir leur propre « niveau de service acceptable » ; Mojaloop fait certains arbitrages :
      1. Lorsque les modes de panne le permettent, le service est dégradé pour l’ensemble des participants plutôt que certains subissent une panne totale pendant que d’autres restent servis.
...
9. Les arbitrages privilégient la disponibilité globale du service et la cohérence d’état plutôt que la performance. C’est-à-dire :
   1. Tous les participants peuvent continuer à transacter à un débit réduit plutôt que certains ne puissent plus transacter du tout.
...
2. En cas de défaillances multiples non couvertes, l’API Mojaloop permet de retrouver un état cohérent avec une exposition financière minimale.
   1. Les transferts ne deviennent juridiquement contraignants que lorsque le hub a répondu au message d’exécution du participant bénéficiaire. Cette réponse n’est envoyée qu’après persistance du message d’exécution et de son résultat dans la base du grand livre.
...
5. Les participants ont besoin d’une information rapide et fiable sur le statut des opérations pour limiter l’exposition et la qualité de service client.
...
2. Architecture distribuée microservices.
    ...
    2. [Séparation des préoccupations](https://en.wikipedia.org/wiki/Separation_of_concerns) garantie par des contrats inter-modules.
...
En s’appuyant sur une approche numérique ouverte des transactions et en s’associant à des acteurs publics et privés, le Level One Project vise un socle partagé robuste et peu coûteux pour les services financiers numériques, stimulant l’innovation, réduisant les risques et créant de la valeur pour les fournisseurs, les particuliers et les économies des marchés en développement. Des ressources complémentaires aident gouvernements, ONG et fournisseurs de services financiers à mettre en œuvre ces changements.
