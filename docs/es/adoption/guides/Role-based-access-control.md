---
i18n_source_sha: ad1dff78324334083e27b3fbf1c6473b9d6d1550
---
...
1. **Permiso** : autoridad para realizar una acción concreta en el contexto de una aplicación o servicio
2. **Rol** : las aplicaciones, las acciones y el acceso a datos necesarios para realizar las tareas relativas a un único rol
3. **Relación usuario-rol** : el rol (o los roles) asignados a cada usuario, que definen los permisos que tiene

## Usuarios, acciones y roles en un contexto de Mojaloop

Mojaloop tendrá 2 grandes categorías de usuarios:

1. Humanos – Son los usuarios del hub y de los DFSP que, a través de distintas interfaces, interactuarán con Mojaloop. Los usuarios de los DFSP interactuarán con Mojaloop a través de Payment Manager y de los portales que se pondrán a disposición durante el proceso de incorporación.
2. No humanos – Estos automatizarán los procesos de negocio y las tareas que de otro modo haría una persona. Se comunicarán mediante llamadas a la API que ejecutarán acciones para cumplir requisitos de negocio.
...
1. Hub: se crearán los siguientes usuarios
  1.1 Operadores del Hub
  1.2. Administradores del Hub
2. DFSP
  2.1 Operadores del DFSP
  2.2 Administradores del DFSP (solo aplicable a Payment manager).
...
2. Racionalizar todos los requisitos de acceso de seguridad de los usuarios para las Aplicaciones de Mojaloop, tal como se describe en la tabla siguiente:
  2.1 Definir las funciones de negocio y de aplicación para los usuarios y las API
  2.2 Definir los perfiles de rol
  2.3 Definir los perfiles de función &amp; competencia
  2.4 Reunir una lista de los conflictos de SOD aplicables definiendo los roles de segregación de funciones
  2.5 Tabla de matriz de perfiles de rol (con datos de ejemplo):
...
3. Reconfigurar los roles que entren en conflicto
  3.1 Aprobaciones de negocio
  3.2 Creación de usuarios maker checker
...
7. Los controles adicionales para mitigar los riesgos del RBAC pueden incluir:
  7.1 Autenticación multifactor (MFA)
  7.2 Registro de auditoría con alertas
  7.3 Desactivación o deshabilitación automática del perfil de usuario
...
10. Identificar y monitorear de cerca las identidades privilegiadas que tienen permisos para realizar acciones sensibles. Lo siguiente se aplica a los usuarios privilegiados:
  10.1 Los privilegios avanzados deben solicitarse y aprobarse caso por caso;
  10.2 Los administradores deberían tener sus permisos privilegiados durante el mínimo tiempo posible;
  10.3 Los administradores solo deberían tener los permisos necesarios para completar una tarea concreta;
  10.4 La pertenencia a los grupos administrativos debe revisarse con regularidad;
  10.5 Imponer la autenticación multifactor a los usuarios administrativos;
  10.6 Conservar los registros de acceso y las auditorías, y configurar notificaciones en tiempo real cuando se active un acceso.
...
El Hub de Mojaloop usa un método de Control de Acceso Basado en Roles (RBAC). Un usuario con una cuenta que permite acceder al Hub tendrá roles asociados a esa cuenta, que definen lo que puede hacer una vez que se ha autenticado y ha iniciado sesión.
...
- **Incorporación del Operador del Hub:**
...
- **Finance Portal:** tiene dos funciones principales: la gestión de las operaciones de liquidación y la gestión de la posición de liquidez de cada DFSP (y, en relación con esto, su valor de Límite de débito neto (NDC)).
...
- **Ciclo de vida del participante:** controlar y configurar el acceso de los DFSP al Hub.
...
- **Operaciones del Hub:** incluyen las búsquedas de transacciones, el monitoreo del estado y del rendimiento, los tableros y las operaciones técnicas en general.
